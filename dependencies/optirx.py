...
def mkcmdsock(ip_address=None, port=PORT_DATA):
...
def mkdatasock(ip_address=None, multicast_address=MULTICAST_ADDRESS, port=PORT_DATA):
    ...
    #Modified by Nikhil for Unicast

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, 0)
    sock.connect((ip_address, port)) ## <--- Using the server address
